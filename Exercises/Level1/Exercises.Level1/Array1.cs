...
public class Array1
{
    ...
    {  
        int first = nums[0];              // Izgūstam masīva pirmo elementu un saglabājam iekš mainīgā first 
        int last = nums[nums.Length - 1]; // Izgūstam masīva pēdējo elementu un saglabājam iekš mainīgā last  
        if (first == 6 || last == 6)      // Ja pirmais vai pēdējais elemets ir vienāds ar 6 tad atgriež vērtību true
        {
            return true;
        }  
            return false;   // Ja pirmais vai pēdējais elements nav vienāds ar 6, tad atgriežam vērtību false
    }
    ...
    {
        // Ja masīva garums ir 0, jeb masīvā nav neviena elementa,tad atgriežam vērtību false 
    ...
    public int[] MakePi()
    {
        // Izveidojam un atgriežam skaitļu masīvu ar garumu 3, kurā ir pirmie 3 pi, { 3, 1, 4} cipari
    ...
    {
       /* Doti 2 skaitļu masīvi, a un b, atgriežam vērtību true, ja tiem ir viens un tas pats pirmais elements vai
            pēdējais.Abu masīvu garums ir 1. */
        ...
        int bLast = b[b.Length - 1];
            ...
            if (aFirst == bFirst || aLast == bLast) return true;  // Mēģinu īsāko koda pieraksta veidu
    ...
    {
        /* Ņemot vērā skaitļu masīva garumu 3, atgrieziet masīvu ar elementiem "rotētu pa kreisi", lai { 1, 2, 3}
        iegūtu { 2, 3, 1}. */ 

        int[] result = { nums[1], nums[2], nums[0] };  // Izveidojam jaunu masīvu secībā: { 2, 3, 1 } 
    ...
    public int[] Reverse3(int[] nums)
    {
        ...
        int[] result = { nums[2], nums[1], nums[0] };  // Izveidojam jaunu masīvu reversā secībā: { 3, 2, 1 }
    ...
    /// </summary>
    /// Ņemot skaitļu masīvu ar garuma 3, noskaidrojiet, kurš ir lielāks, masīva pirmais vai pēdējais elements un
    /// iestatiet visus pārējos elementus kā šo vērtību. Atgrieziet mainīto masīvu.
public int[] MaxEnd3(int[] nums)
{
    ...
    for (int i = 0; i < nums.Length; i++)  // Nomainām katra masīva elementa vērtību uz atrasto lielāko vērtību
        {
        nums[i] = biggest;
    }
  
    return nums; // Atgriežam modificēto masīvu
